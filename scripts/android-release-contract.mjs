...
must(candidate,'ANDROID_VERSION_NAME: "1.0.1"','release version name');
must(candidate,'ANDROID_VERSION_CODE: "2"','release version code');
...
must(candidate,'Verify SecureStore registration order','native plugin registration CI');
must(trusted,'ANDROID_KEYSTORE_BASE64','trusted release signing secret contract');

console.log('Android 1.0.1 release contract passed.');

...
export function LiveAccountScreen(){
  ...
  const resource=useApiResource(async()=>{
    ...
    let admin=false;
    try{admin=(await api.adminMe()).roles.includes('super_admin');}catch{/* regular accounts are not staff */}
    ...
    return {profile:profile.profile,wallet,equipment:equipment.equipment,androidUpdate,admin};
  },[]);
  ...
  return (
    <main className="page-shell">
      ...
      <section className="account-shortcuts"><Link to="/notifications" className="link-reset"><Icon name="bell"/><span>الإشعارات</span></Link><Link to="/store" className="link-reset"><Icon name="store"/><span>مشترياتي</span></Link><Link to="/account/devices" className="link-reset"><span>📱</span><span>أجهزتي</span></Link><Link to="/account/prayer" className="link-reset"><span>🌙</span><span>الصلاة</span></Link>{resource.data?.admin?<Link to="/admin" className="link-reset"><span>🛡️</span><span>الإدارة</span></Link>:null}</section>
